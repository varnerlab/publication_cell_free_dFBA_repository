...
dataset_index = Array{Int64}(readdlm("FVA_SA/species_boolean_array"))
(n_species,n_ens) = size(dataset_index)
...
Labels = String[]
for i in 1:n_ens
	#species_constraint_array[i,1:end] = dataset_index

	push!(Labels,"SA_$i")
end

#num_constraint_sets = size(species_constraint_array,1)
num_constraint_sets = n_ens
# not currently used
...
for constraint_index in constraint_index_array[1:end] ;println(Labels[constraint_index])
	...
	if !isdir("FVA_SA/$constraint_subset")
		mkdir("FVA_SA/$constraint_subset")
	end
	...
	species_constraint_index = find(Array{Int64}(species_constraint_array[:,constraint_index]))
  println(species_constraint_index)
#	species_constraint_index = copy(dataset_index)
  ...
  writedlm("FVA_SA/$constraint_subset/constraint_subset",species_constraint_index)
	...
	for rxn_idx in 1:194
		if !isdir("FVA_SA/$constraint_subset/$rxn_idx")
			mkdir("FVA_SA/$constraint_subset/$rxn_idx")
		end
		writedlm("FVA_SA/$constraint_subset/$rxn_idx/FVA_min",FVA_min[rxn_idx])
