  ...
  for j in 1+i:6
    for k in j+i:6
      println(index)
    	...
    	species_constraint_array[index,end-1] = combo_species_additions[i,1]
      species_constraint_array[index,end] = combo_species_additions[j,1]
    	push!(Labels,"Plus_"*combo_species_additions[i,2]*"_"*combo_species_additions[j,2])
      index = index+1
    end
  end
end
